...
def init_catalog(config: CatalogConfig) -> CatalogManifest:
    """Create the generated .catalog workbench without indexing sources."""
...
def catalog_status(config: CatalogConfig) -> CatalogStatus:
    ...
    if not config.catalog_dir.exists() or not manifest_exists:
        return CatalogStatus(
            ...
            stale_reasons=[".catalog has not been initialized."],
    ...
    if missing_artifacts:
        stale_reasons.append("One or more required .catalog artifacts are missing.")
...
def write_catalog_ai(config: CatalogConfig, manifest: CatalogManifest) -> None:
    ...
    lines = [
        "---",
        'title: "AI.md - Catalog Derived State"',
        "doc_type: ai-entry",
        "generated_by: catalog",
        "---",
        "",
        "# AI.md - Catalog Derived State",
        "",
        "This directory is generated by Catalog. Treat source corpus files as canonical.",
        "",
        "## What Lives Here",
        "",
...
def gitignore_warning(config: CatalogConfig) -> str | None:
    ...
    if not gitignore.exists():
        return "Catalog did not edit .gitignore; consider ignoring .catalog/."
